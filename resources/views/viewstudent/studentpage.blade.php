				...
				<div class="card-body">
					<div class="d-flex flex-wrap align-items-center justify-content-between gap-2">
						<div>
							...
							<p class="text-muted mb-0">Here is your timetable.</p>
						</div>
						<a href="{{ route('student.settings.edit') }}" class="btn btn-outline-secondary rounded-3">
		...
		<div class="card-body">
			@if ($timetables->isEmpty())
				<div class="alert alert-info border-0 rounded-3 mb-0">
					No timetable records found yet.
				</div>
			@else
					...
					<table class="table align-middle mb-0">
						...
						@foreach ($timetables as $row)
							<tr>
								...
								<td>{{ $row->subject?->lecturer_name ?? '-' }}</td>
								<td>
									@if($row->hall)
